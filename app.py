...
def migrate_passwords_to_hash():
    """Ensures all user passwords in the DB are securely hashed using werkzeug."""
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        if migrated_count > 0:
            conn.commit()
            print(f"[SECURITY] Successfully migrated {migrated_count} legacy plaintext passwords to secure werkzeug hashes.")
        conn.close()
    except Exception as e:
        print("[SECURITY] Password migration check error:", e)
...
def seed_database(cursor):
    ...
    VALUES (1, 'admin', ?, 'admin', NULL, 'Executive Admin')
    ...
    for d in depots_data:
        ...
        INSERT OR IGNORE INTO users (id, username, password, role, depot_id, display_name)
        VALUES (?, ?, ?, 'incharge', ?, ?)
        ''', (uid, username, generate_password_hash('depot123'), d[0], display_name))
...
if not os.path.exists(DB_PATH):
    ...
    if c.fetchone()[0] == 0:
        ...
        migrate_passwords_to_hash()  # No auto-seeding of demo data — DB stays clean until real data is entered
    conn.close()
...
def api_login():
    ...
    if user_row:
        ...
        stored_pwd = user_dict.get('password', '')
        ...
        if stored_pwd.startswith('pbkdf2:') or stored_pwd.startswith('scrypt:'):
            ...
            try:
                ...
                conn.execute("UPDATE users SET password = ? WHERE id = ?", (new_hash, user_dict['id']))
                conn.commit()
            except Exception:
                pass
...
def change_user_password():
    ...
    old_matches = False
    if stored_pwd.startswith('pbkdf2:') or stored_pwd.startswith('scrypt:'):
        old_matches = check_password_hash(stored_pwd, old_password)
    elif stored_pwd == old_password:
        old_matches = True
...
def reset_user_password():
    ...
    if user_id:
        if new_username:
            cursor.execute('UPDATE users SET username = ?, password = ? WHERE id = ?', (new_username.lower().strip(), hashed_pwd, user_id))
        else:
...
# ------------------------------------------------------------------------------
# NOTE: a "/api/sample-poloxy-data" demo-data route + private_data/sample_poloxy_data.json
# used to live here. It only fed an unused browser-console-only helper
# (loadSamplePoloxyDataDirectly) that no visible button ever called, so both the
# route and the JSON file (which held real-looking customer names/phones/addresses)
# were removed rather than kept around for no real feature.
# ------------------------------------------------------------------------------
...
def export_master_excel():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to generate live master excel: {str(e)}"}), 500
...
def create_admin_user():
    ...
    if role != 'admin' and not depot_id:
        return jsonify({"success": False, "message": "Please select a depot for this user."}), 400
    if not re.match(r'^[a-z0-9_.-]+$', username):
        return jsonify({"success": False, "message": "Username may only contain letters, numbers, dots, hyphens and underscores."}), 400

    conn = get_db()
    cursor = conn.cursor()

    existing = cursor.execute(
        "SELECT id FROM users WHERE REPLACE(LOWER(username), '-', '_') = REPLACE(?, '-', '_')",
        (username,)
    ).fetchone()
    if existing:
        conn.close()
        return jsonify({"success": False, "message": f"Username '{username}' is already taken. Please choose another."}), 409

    try:
        cursor.execute('''
            INSERT INTO users (username, password, role, depot_id, display_name, is_active)
            VALUES (?, ?, ?, ?, ?, 1)
        ''', (username, generate_password_hash(password), role, depot_id if role != 'admin' else None, display_name))
        conn.commit()
